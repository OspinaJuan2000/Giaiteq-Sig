    ...
    fetch('../../modelo/modeloDocumentos/subir-documento.php', {
        ...
        .then(data => {
            ...
            if (mensaje === 'sindocumento') {
                ...
                tituloAlerta = 'Solo se permiten los fórmatos .DOC, .DOCX, .XLSX, .PPTX y .PDF';
                mensajeAlerta = 'Intente nuevamente subiendo el documento en el fórmato solicitado';
                iconoAlerta = 'info';
    ...
    fetch('../../modelo/modeloDocumentos/listar-documentos.php')
        ...
        .then(data => {
            ...
            if (mensaje === 'sin_documentos') {
                ...
                funciones.mensajeSinDocumentos('No se ha subido ningún documento', 'sinRegistrosBD');
            } else {
    ...
    fetch('../../modelo/modeloDocumentos/eliminar-documento.php', {
        ...
        .then(data => {
            ...
            if (mensaje === 'documento_eliminado') {
                ...
                mensajeAlerta = `Se ha eliminado el documento ${data.titulo}`;
                iconoAlerta = 'success';
                console.log(contenedorDocumentos);

            } else if (mensaje === 'documento_noeliminado') {
                tituloAlerta = 'Error al intentar eliminar';
                mensajeAlerta = `Ocurrió un error al intentar eliminar el documento ${data.titulo}`;
                iconoAlerta = 'error';
            }
    ...
    fetch('../../modelo/modeloDocumentos/buscar-documentos.php', {
        ...
        .then(data => {
            ...
            if (mensaje === 'documento_noencontrado') {
                funciones.mensajeSinDocumentos('No se ha encontrado ningún documento con el título ingresado', 'sinFiltrosBD');
            } else {
                funciones.renderizarListaDocumentos(data);
            }
        }).catch(err => console.log(err));
